<template lang="pug">
    section(class="bg-[#F8F5F3] pt-[24px] lg:pt-[64px] pb-[80px]")
        .app-container.relative
            .breadcrumb.flex.lime-clamp-1.gap-2.items-center(class="text-[14px] font-300")
                ...
                NuxtLink(:to="localePath('/subscriptions')") {{ t('subscriptions') }}
                IconArrowLeft(class="text-[#5D5A57] ltr:rotate-180")
            ...
            div(class="lg:absolute  lg:w-[50px] top-[5.3rem]" :style="windowWidth > 1024 ? { height: containerHeight + 200 + 'px' } : {}" ref="el")
                ProgramDetailsCard.block(v-model="addedToCart" :loading="addToCartLoading" type="subscription" :data="track" ref="cardRef" :amount="track?.accessDurationPerMonths" @addToCart="handleAddToCart" @buyNow="handleBuyNowPopup" class="mt-5 shadow-sm lg:mt-0 lg:sticky top-[24px] rtl:2xl:mr-[68.2em] rtl:xl:mr-[52.2rem] rtl:lg:mr-[36.2rem] ltr:2xl:ml-[68.2em] ltr:xl:ml-[52.2rem] ltr:lg:ml-[36.2rem]")
    ...
    section(class="bg-white pt-[64px] pb-[80px]")
        .app-container
            div(class="w-full lg:max-w-[50%]" ref="containerRef")
                div
                    p(class="text-[24px] font-400") {{ t('aboutSubscription') }}
                    //- div(class="text-[16px] mt-5 font-light break-words whitespace-pre-wrap max-w-none [&_code]:whitespace-pre-wrap [&_code]:break-words" v-html="locale === 'en' ? track?.diplomaDetails?.enAbout : track?.diplomaDetails?.arAbout")
                    div(class="content-area text-[16px] mt-5 font-light break-words whitespace-pre-wrap max-w-none [&_ul]:list-disc [&_ul]:ml-6 [&_ul]:mb-4 [&_li]:mb-2 [&_p]:mb-4 rtl:[&_p]:!text-right ltr:[&_p]:!text-left " v-html="locale === 'en' ? track?.diplomaDetails?.enAbout : track?.diplomaDetails?.arAbout")

                div(class="mt-[64px]")
                    p(class="text-[24px] font-400") {{ t('subscriptionContent') }}
                        ...
                        template(#accordion-item="{ item }")
                            .flex.items-center(class="gap-[16px]")
                              ...
                              div
                                ...
                                p(class="text-[14px]  w-fit mt-2  text-primary ") {{ t(item?.type?.toLowerCase()) }}
    ...
    section(class="py-12" v-if="recommendedSubcreptions?.length")
      .app-container
        BaseSection(:title="t('moreSubscriptionsYouMightLike')" link="/subscriptions" :scrollHorizontalOnSmScreen="true" :linkTitle="t('allSubscriptions')" :gridCols="3")
              template(v-for="path in recommendedSubcreptions?.slice(0, 3)" v-if="!loading")
                  DiplomaCard(:data="path"  class="flex-shrink-0 w-[250px] md:w-full" type="subscriptions")
...
const {
  data: recommendedSubcreptions,
  error,
  status,
} = useAsyncData(
  `recommended-subscriptions-${route.params.id}`,
  () =>
    GqlRecommendedDiplomas({
      diplomaId: track?.value?.id,
      filter: { diplomaType: DiplomaTypeEnum.SUBSCRIPTION },
    }),
...
useSchemaOrg([
  defineCourse({
    ...
    url: config.public.APP_BASE_URL + `/subscriptions/${track?.value?.slug}`,
